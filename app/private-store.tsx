...
export function PrivateStore({ children }: { children: ReactNode }) {
  ...
  return <Store.Provider value={context}>
    ...
    {ready && paired ? children : <main className="private-entry"><h1>个人工作台</h1><p>{ready ? "连接你的私人空间" : "正在连接你的工作台"}</p>{ready && <><label>{recoveryMode ? "恢复码" : "配对码或配对链接"}<input type="password" value={code} onChange={e => { const raw = e.target.value; try { const url = new URL(raw); setCode(new URLSearchParams(url.hash.slice(1)).get("pair") || raw); } catch { setCode(raw); } }} autoComplete="off" /></label><button onClick={() => void run(pair)}>连接此设备</button><button onClick={() => setRecoveryMode(!recoveryMode)}>{recoveryMode ? "使用配对码" : "使用恢复码"}</button></>}</main>}
